...
function CreateProblemInstance(E,T,Dmin,Dmax,α,p,Q,Vmin,Vmax,v1,optimize_model)
    ...
    @constraint(model, Obj_Eq[s=1:T], ε_pos[s] - ε_neg[s]  == sum(sum(α[t][e]*y[t,s,e] for t=s:T) for e=1:E), set_string_name = false)
    ...
    @constraint(model, Cap_Eq[t=1:T,s=1:t,e=1:E], y[t,s,e] == γ_pos[t,s,e] - γ_neg[t,s,e], set_string_name = false)
    @constraint(model, Cap_UB[t=1:T,e=1:E], sum(Dmax[s]*γ_pos[t,s,e] - Dmin[s]*γ_neg[t,s,e] for s=1:t) ≤ p[t][e], set_string_name = false)
    @constraint(model, Cap_LB[t=1:T,e=1:E], sum(-Dmin[s]*γ_pos[t,s,e] + Dmax[s]*γ_neg[t,s,e] for s=1:t) ≤ 0, set_string_name = false)
    println("Create capacity constraints: ", now() - start_time)
    flush(stdout)
    ...
    @constraint(model, Fac_Eq[s=1:T,e=1:E], sum(y[t,s,e] for t=s:T) == δ_pos[s,e] - δ_neg[s,e], set_string_name = false)
    @constraint(model, Fac_UB[e=1:E], sum(Dmax[s]*δ_pos[s,e] - Dmin[s]*δ_neg[s,e] for s=1:T) ≤ Q[e], set_string_name = false)
    ...
    @constraint(model, Tot_Eq[s=2:T,t=s:T], -1 + sum(sum( y[ℓ,s,e] for e=1:E) for ℓ=s:t) == η_pos[t,s] - η_neg[t,s], set_string_name = false)
    @constraint(model, Tot_UB[t=1:T],  v1 + sum(sum(y[s,1,e] for e=1:E) for s=1:t) + sum( Dmax[s]*η_pos[t,s] - Dmin[s]*η_neg[t,s] for s=2:t) - Dmin[t+1] ≤ Vmax, set_string_name = false)
    @constraint(model, Tot_LB[t=1:T], -v1 - sum(sum(y[s,1,e] for e=1:E) for s=1:t) + sum(-Dmin[s]*η_pos[t,s] + Dmax[s]*η_neg[t,s] for s=2:t) + Dmax[t+1] ≤ - Vmin, set_string_name = false)
    println("Create total constraints: ", now() - start_time)
    flush(stdout)
